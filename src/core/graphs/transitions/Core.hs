{-# LANGUAGE DeriveFunctor, DeriveFoldable, DeriveTraversable, DeriveDataTypeable, TupleSections, RecordWildCards, FlexibleInstances, BangPatterns, LambdaCase #-}
...
tagName s =
  case s of
    ...
    Use _ _ -> "Use"
    Assign _ _ -> "Assign"
    Dup (Level l) _ _ -> "Dup" ++ show l
    DupInv (Level l) _ _ -> "DupI" ++ show l
    Lift _ _ -> "Lift"
    LiftInv _ _ -> "LiftI"
...
mkDot :: [Syntax] -> [(Edge, Syntax)] -> String
          ...
          Just left_node ->
            case (right_node,left_node) of
              ...
              (PiR (Rule pr_cases (Sequent pr_bseq pr_bl (pr_main, pr_br))),
                ...
                pure . join_edges (zip pr_m_tl pr_bl ++ zip pr_m_tr pr_br) .
...
writeFirst = do
  ...
  let f = mkDot graph [(fromJust $ findEdge (edges graph) (EID "ret1"),last graph)]
  writeFile "graphs/test.dot" f

findRoot graph = case find (\case Assign _ _ -> True; _ -> False) graph of
  Nothing -> error $ "no Assign?!"
...
toDot :: Graph -> [Maybe (EID, GDir)] -> State Int [String]
toDot graph rs = do
  graphs <- iterGraph graph rs
  pure $ map (\(g,a) -> mkDot g a) graphs
...
writeGraphs graph name limit = do
  ...
  let rs = replicate 3 Nothing ++ [Just (EID "lval38", Up)] ++ repeat Nothing
