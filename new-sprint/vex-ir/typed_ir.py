...
class Derivation:
    ...
    def __post_init__(self) -> None:
        ...
        for i, (prem, top) in enumerate(zip(self.premises, tops)):
            if prem.instantiated_rule.bottom != top:
                raise ValueError(
                    f"Premise {i} conclusion {prem.instantiated_rule.bottom} "
                    f"does not match required top {top}"
                )
    ...
    def pretty(self, indent: int = 0) -> str:
        ...
        lines.append(f"{pad}[{self.instantiated_rule.rule}]  {self.conclusion}")
        return "\n".join(lines)
