...
1. Instruction selection - Peephole optimization, Common subexpression elimination
2. Register allocation - Spilling, Rematerialization
3. Instruction scheduling - Code motion, Block ordering
...
Data hazards: RAW is unavoidable. WAR/WAW are eliminated in modern processors by renaming as in the [Tomasulo algorithm](https://en.wikipedia.org/wiki/Tomasulo_algorithm). WAW can be also ignored if the value isn't used.
...
We can use direct or indirect threading with computed goto. clang/LLVM optimizes the looped switch to indirect threading at `-O`. ([ref](https://internals.rust-lang.org/t/computed-gotos-tco-threaded-interpreters-experiments-and-findings/4668/6))
