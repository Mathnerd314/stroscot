...
To show how optimal reduction works, we step through the example term `M = o $ \h. o (h I)` from pages 26-35 of {cite}`aspertiOptimalImplementationFunctional1999`. To write the derivation tree we must define the recursive type $\Omega = \, !\Omega \to \Omega$.

```{image} /images/Stroscot_M_Proof_Tree.svg
```
