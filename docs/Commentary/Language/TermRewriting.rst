...
Actually there is a larger relation. We can define a family of relation :math:`\overset{*}{\to}_\epsilon` which contains the single-step relation, is transitively closed, and in addition to containing the reflexive relation, contains all pairs :math:`(x,x')` with :math:`d(x,x')<\epsilon`. We see that each :math:`\overset{*}{\to}_\epsilon` is topologically closed since if we have a sequence :math:`(x_n, y_n) \to (x, y)` then we can just go far enough to have :math:`d(x,x_n)<\epsilon` and :math:`d(y,y_n)<\epsilon` and then we conclude :math:`x \overset{*}{\to}_\epsilon y` from :math:`x_n \overset{*}{\to}_\epsilon y_n`.

Since (due to our distance metric) all distances are of the form :math:`2^{-n}`, we can limit our considerations to the sequence of relations :math:`R_n = \overset{*}{\to}_{2^{-n}}`. :math:`R_n` is monotonically decreasing in the sense that :math:`R_n \supseteq R_{n'}` for :math:`n<n'`, so there is a pointwise convergence property: either :math:`(x,y) \in R_n` for all :math:`n`, or there is an :math:`n` such that :math:`(x,y) \notin R_{n'}` for :math:`n' > n`. Therefore, we can define :math:`E^*(\to) = \lim_{\epsilon\to 0} \overset{*}{\to}_\epsilon = \lim_{n\to \infty} R_n = \bigcap_n R_n` (the limit here being something like the `Kuratowski limit <https://en.wikipedia.org/wiki/Kuratowski_convergence>`__).

Does E* = T*? Well, :math:`T^*(\to) \subseteq R_n`, because they are closed under the properties of T* and also the epsilon-distance.  Therefore, :math:`T^*(\to) \subseteq E^*(\to)`. What about :math:`E^*(\to) \subseteq T^*(\to)`? Well... it is tricky. For each :math:`n`, we have a rewrite sequence of alternating epsilons and single-steps, connected by transitivity. We don't need to worry about more than one epsilon between each single-step, as our metric is an ultrametric. Similarly we can assume an epsilon around each single-step because it's simply distance zero. So for each :math:`\epsilon` we have a (finite) rewrite sequence like epsilon-step-epsilon-step-epsilon. But it is not clear that we can take these disconnected segments and get a relation in T* using topological closure and transitivity. Like say we have binary numbers ``0 1 0 1`` and the single-step rewrite relation takes a term like ``0 1 0 1 +`` to ``0 1 1 0``, incrementing by 1. With E*, starting at ``0 0 0 0 ...``, we can go within epsilon to a finite term ending in ``+``, and single-step plus epsilon up to ``1 1 1 ... 1``, so there is the rewrite ``0 0 0 ... -> 1 1 1 ...``. But with T* it seems ``0 0 0 0 ...`` does not rewrite to anything besides itself, because the only rewrite relations within epsilon of ``0 0 ...`` are ``0 ... 0 + = 0 ... 1`` and the RHS also converges to ``0 0 ...``. It would be nice if I have misinterpreted something and they are the same but for now I am going with E* as the definition.
...
We also have that :math:`R_n(x)` converges pointwise to :math:`E^*(x)` using the Hausdorff distance because, considering :math:`\sup_{b\in R_n(x)}d(E^*(x),b)`, there is an :math:`n` that is sufficiently large that no points more than :math:`\epsilon` away from :math:`\overset{*}{\to}_0(x)` disappear. Specifically, for each point :math:`b \notin E^*(x)` there is an :math:`m` such that for :math:`m' > m`, :math:`b \notin R_{m'}(x)`. Now suppose for contradiction that no such :math:`n`  exists, i.e. for every :math:`n` there is a point :math:`b_n \in R_n(x)` with :math:`d(b_n,E^*(x))>\epsilon`. Then since the set of terms is compact we have that :math:`b_n` has an accumulation point, say :math:`b`. But since :math:`b \in R_n(x)` we must have :math:`b \in E^*(x)` so then :math:`d(b_n,E^*(x))<d(b_n,b)+d(b,E^*(x)) < \epsilon`. Therefore, such an :math:`n` exists such that :math:`d(R_n(x),E^*(x))<\epsilon`.

There is also the notion of the length of a rewrite sequence, for example as follows:

* The length of a reflexive rewrite ``a -> a`` or epsilon rewrite :math:`d(x,x') < \epsilon` is 1
...
* The length of a topological rewrite ``a -> b`` where there is a sequence of rewrites :math:`a_i \to b_i` and :math:`\lim a_i = a` and :math:`\lim b_i = b` is the supremum of the lengths of :math:`a_i \to b_i`, plus 1.
...
For proving things like confluence, we need a strong property similar to uniform convergence. Dini's theorem seems relevant. But first we need to define what it means for :math:`\overset{*}{\to}` to be "continuous". There was the suggestion in `MathOverflow <https://mathoverflow.net/questions/179123/continuous-relations>`__ to consider whether the function :math:`R(x) = \{ y : x \overset{*}{\to} y\}` is continuous. As far as the distance metric in :math:`P(S)`, ChatGPT suggested to use the `Hausdorff distance <https://en.wikipedia.org/wiki/Hausdorff_distance>`__. Unfortunately this doesn't work: we may have the rewrite rules ``A B = B; A C = C`` and then ``x = A (A (A (...)))`` and ``x_n = A (A (A (... (B))))`` (n initial A's). We have ``R(x) = {B,C,A B,A C,A (A B),...}`` and ``R(x_n) = {B,A B,A (A B), ..., A (... (A B))}``. Clearly ``C`` is distance 1 from every element of ``R(x_n)``, so the Hausdorff distance of ``R(x_n)`` and ``R(x)`` is 1 even though the distance between ``x`` and ``x_n`` goes to zero, so ``R`` isn't continuous under this definition.

The next suggestion is that a relation is continuous if it preserves limits. This is analogous to how one can define continuity for functions between topological spaces using nets; one requires that for any net :math:`(x_a)_{a\in A}` with :math:`\lim x_a = c`, the corresponding net :math:`\left(y_a = f(x_a)\right)_{a\in A}` has :math:`\lim_{a\in A} y_a = f(c)`. But this definition for functions is not directly applicable to relations because a relation may have multiple values and multiple limits. Therefore, the author's suggestion is to consider accumulation points: one requires that for any net :math:`(x_a)_{a\in A}` with an accumulation point :math:`x`, and a corresponding net :math:`\left(y_a \in R(x_a)\right)_{a\in A}`, then there exists a point :math:`y \in R(x)` that is an accumulation point of :math:`y_a`. The author discusses a bit about how restricting to ultranets is an equivalent definition and allows using limits instead of accumulation points, but constructing non-constant ultranets requires the axiom of choice, so I will avoid that. Now this definition is a bit too general for our purposes. Since we are dealing with a metric space, and it is first-countable, every net corresponds to a sequence (a net with countable basis). So we can assume :math:`A=\mathbb{N}`. So what is required to prove is that for any sequence :math:`(x_n)` with an accumulation point :math:`x`, and a corresponding sequence :math:`\left(y_n \in E^*(\to)(x_n)\right)`, then there exists a point :math:`y \in E^*(\to)(x)` that is an accumulation point of :math:`y_n`.



 That is, suppose :math:`\lim x_n \to x` (where :math:`\lim a_n \to b` means :math:`b` is an accumulation point of the set :math:`a_n`) and there exists a sequence :math:`y_n`, with :math:`y_n \in R(x_n)`. Then "preserving the limit" means there is a point :math:`y` such that :math:`\lim y_n \to y` and :math:`y \in R(x)` . For E* this is true because the space of terms is compact, so there is always a convergent subsequence of :math:`y_n`
...
We could also just consider the Hausdorff distance on :math:`S \times S` but the function-to-power-set construction is more natural IMO.

So going back to Dini's theorem: the set S of terms is a compact metric space. Our relations :math:`R_n` are also monotonically decreasing in that :math:`R_n(x) \supseteq R_{n+1}(x)`. Each :math:`R_n` is continuous in that for any :math:`\epsilon > 0`, we can choose :math:`\delta = 2^{-n}` and then if :math:`d(x,x')<\delta` then :math:`d(R_n(x),R_n(x'))=0`.

Finally we must show that :math:`R` is continuous. Take :math:`x` and :math:`\epsilon > 0`, and suppose for contradiction that for every :math:`\delta > 0` there is an :math:`x'` with :math:`d(x,x')<\delta` and :math:`d(R(x),R(x'))\geq\epsilon`. Then either there is a point :math:`y \in R(x)` with :math:`d(y,R(x')) > \epsilon/2` or a point :math:`y' \in R(x')` with :math:`d(R(x),y') > \epsilon/2`. Now consider a sequence :math:`x_n` of such :math:`x`'s, for :math:`\delta = 2^{-n}`. We must either have infinitely many :math:`y_n \in R(x)` or infinitely many :math:`y_n \in R(x_n)`. Suppose for the moment there are infinitely many :math:`y_n \in R(x)` and the subsequence of such :math:`n`. Then as :math:`R(x)` is closed we must have :math:`y = \lim y_n \in R(x)`. So we have :math:`x \overset{*}\to y`, :math:`\lim x_n = x`, but :math:`d(y,R(x_n)) > \epsilon/4`. Observe that :math:`x` must be an infinite term because if :math:`x` is finite there are no other points within the distance bounded by the depth of the term. So for example,

 but no ``x_n`` rewrites to anything starting with ``C``. So then we conclude that :math:`R` is not continuous

Now considering the alternative, infinitely many :math:`y_n' \in R(x_n')`,


Consider  or :math:`y_n \in R(x')` and :math:`d(R(x),y_n) > \epsilon/2`

 Take the subsequence of such :math:`(x'_n,y_n`
...
Proof
Let ε > 0 \varepsilon >0 be given. For each n ∈ N n\in \mathbb {N} , let g n = f − f n {\displaystyle g_{n}=f-f_{n}}, and let E n E_{n} be the set of those x ∈ X x\in X such that g n ( x ) < ε {\displaystyle g_{n}(x)<\varepsilon }. Each g n g_{n} is continuous, and so each E n E_{n} is open (because each E n E_{n} is the preimage of the open set ( − ∞ , ε ) {\displaystyle (-\infty ,\varepsilon )} under g n g_{n}, a continuous function). Since ( f n ) n ∈ N {\displaystyle (f_{n})_{n\in \mathbb {N} }} is monotonically increasing, ( g n ) n ∈ N {\displaystyle (g_{n})_{n\in \mathbb {N} }} is monotonically decreasing, it follows that the sequence E n E_{n} is ascending (i.e. E n ⊂ E n + 1 {\displaystyle E_{n}\subset E_{n+1}} for all n ∈ N n\in \mathbb {N} ). Since ( f n ) n ∈ N {\displaystyle (f_{n})_{n\in \mathbb {N} }} converges pointwise to f f, it follows that the collection ( E n ) n ∈ N {\displaystyle (E_{n})_{n\in \mathbb {N} }} is an open cover of X X. By compactness, there is a finite subcover, and since E n E_{n} are ascending the largest of these is a cover too. Thus we obtain that there is some positive integer N N such that E N = X {\displaystyle E_{N}=X}. That is, if n > N n>N and x x is a point in X X, then | f ( x ) − f n ( x ) | < ε {\displaystyle |f(x)-f_{n}(x)|<\varepsilon }, as desired.

and that :math:`f : S -> P(S), f(x) = \{y : x \overset{*}{\to} y\}` is a continuous mapping. It seems intuitively true that these hold for finite rule systems, as they can only match on a finite prefix of the term, but with conditionals the conditions may not be continuous.
