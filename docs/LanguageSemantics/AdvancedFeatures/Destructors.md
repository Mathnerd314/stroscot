...
An example usage of destructors is based on [AutoCloseFD](https://android.googlesource.com/platform/system/vold/+/android-7.1.1_r11/AutoCloseFD.h). `AutoCloseFD` is not exported so as to strengthen encapsulation and in particular to avoid separating the destructor and the file descriptor.
