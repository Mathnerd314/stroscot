...
Abstract data types are sets containing trees of uninterpreted symbols. So a datatype declaration (from [here](https://github.com/UlfNorell/insane/blob/master/Context.agda))
...
The types can bind the value, so Stroscot can express dependent types. And the values are in scope in the type, so even [insanely dependent types](https://github.com/UlfNorell/insane/) can be defined:
