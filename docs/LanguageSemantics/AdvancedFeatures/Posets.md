...
Considering `\x y -> comp x y /. { GreaterThan -> Incomparable, LessThan -> Incomparable }`, we obtain a [partial equivalence relation](https://en.wikipedia.org/wiki/Partial_equivalence_relation), hence the name.
...
[IEEE 754](https://grouper.ieee.org/groups/msc/ANSI_IEEE-Std-754-2019/background/predicates.txt) omits `=~` and `<>` as "insufficiently useful to be mandatory", but they seem worth including for completeness.
...
There's also Zarf's [Precedes thing](https://eblong.com/zarf/essays/rule-based-if/ruleday-log.html):
